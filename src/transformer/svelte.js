...
class SvelteTransformer {
  ...
  constructor(content, ast, dir, fileName, moduleName, isDefault) {
    ...
    this.#typeChecker = ts
      .createProgram({
        rootNames: [this.#sourceFile.fileName],
        options: {
      ...
      if (declaration.type) {
        ...
        if (ts.isTemplateLiteral(initializer)) {
          ...
          const initializerType =
            this.#typeChecker.getTypeAtLocation(initializer);
          type = this.#typeChecker.typeToString(initializerType);
        } else if (ts.isCallOrNewExpression(initializer)) {
          const signature = this.#typeChecker.getResolvedSignature(initializer);
          if (signature) {
            type = this.#typeChecker.typeToString(signature.getReturnType());
          }
        } else if (ts.isIdentifier(initializer)) {
        } else {
          type = getSyntaxKindString(declaration.initializer.kind);
        }
      }
